...
float Metrics::RootMeanSquaredError::calculate(const VectorXf &predictions, const VectorXf &target)
{
    const int predictions_length = predictions.size();
    const int target_length = target.size();
    ...
    return sqrt((predictions - target).squaredNorm() / predictions.size());
}