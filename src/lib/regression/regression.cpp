...
Regression::LinearRegression::LinearRegression(float learning_rate,
                                               int iterations,
                                               Metrics::Type metrics,
                                               Optimizer::Type optimizer)
    : learning_rate(learning_rate), iterations(iterations), metrics(metrics),
      optimizer(optimizer) {}

void Regression::LinearRegression::fit(const MatrixXf &data,
                                       const RowVectorXf &target) {
  ...
  for (int i = 0; i < Regression::LinearRegression::iterations; i++) {
    ...
    // cout << "(Iteration " << i << ") " << "Loss: " <<
    // Regression::LinearRegression::loss(data) << "\n";
  }
}

RowVectorXf Regression::LinearRegression::predict(const MatrixXf &data) {
  RowVectorXf predictions =
      data * Regression::LinearRegression::coefficients.transpose();
  RowVectorXf interceptVector = RowVectorXf::Constant(
      data.rows(), Regression::LinearRegression::intercept);
...
void Regression::LinearRegression::optimize() {
  ...
  RowVectorXf coef_error_vector =
      Regression::LinearRegression::data.transpose() * error.transpose();
  Regression::LinearRegression::coefficients =
      Regression::LinearRegression::coefficients -
      (Regression::LinearRegression::learning_rate * coef_error_vector /
       length);
...
float Regression::LinearRegression::loss(const MatrixXf &data) {
  if (Regression::LinearRegression::metrics == Metrics::Type::MSE)
    return Metrics::MeanSquaredError::calculate(
        Regression::LinearRegression::predict(data),
  ...
  if (Regression::LinearRegression::metrics == Metrics::Type::RMSE)
    return Metrics::RootMeanSquaredError::calculate(
        Regression::LinearRegression::predict(data),
        Regression::LinearRegression::target);
}